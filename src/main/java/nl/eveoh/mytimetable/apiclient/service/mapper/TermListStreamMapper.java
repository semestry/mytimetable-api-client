 ...
 * @author Marco Krikke
 */
public class TermListStreamMapper extends StreamMapper<List<Term>> {

    public TermListStreamMapper(ObjectMapper mapper) {
        super(mapper);
    }

    @Override
    public List<Term> map(InputStream stream) {
        try {
            return mapper.reader()
                    .with(DeserializationFeature.UNWRAP_ROOT_VALUE)
                    .withRootName("terms")
                    .forType(mapper.getTypeFactory().constructCollectionType(List.class, Term.class))
                    .readValue(stream);
        } catch (IOException e) {
